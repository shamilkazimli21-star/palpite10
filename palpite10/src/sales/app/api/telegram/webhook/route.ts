...
async function onChannelMembership(u: MemberUpdate): Promise<void> {
  ...
  if (sameChannel(u.chat, env.TELEGRAM_VIP_CHANNEL_ID)) {
    ...
    if (is && !staff && !lead?.vip_active) {
      await notifyAdmin(`⚠️ ${user.first_name ?? "Someone"} (@${user.username ?? "-"}, id ${user.id}) joined the VIP channel WITHOUT an active membership. A link may have been shared.`);
    }
  }
}
...
async function onCallback(q: NonNullable<Update["callback_query"]>, replied: () => Promise<void>): Promise<void> {
  ...
  if (lead.free_channel_joined) return answerCallback(q.id, "Você já está no canal ✅");
  ...
  try {
    ...
    if (await allowRequest("alert:free-channel-check", 1, 3600)) {
      await notifyAdmin(`🚨 Cannot verify free-channel membership: ${(error as Error).message}. Make the bot an ADMIN of the free channel and check TELEGRAM_FREE_CHANNEL_ID.`);
    }
    return;
  }
  if (!member) {
...
async function onMessage(m: Message, replied: () => Promise<void>): Promise<void> {
  ...
  await withLeadLock(found.id, async () => {
    ...
    if (command === "/parar" || command === "/stop" || (!command && isOptOut(text))) {
      await recordMessage(lead.id, "user", text.slice(0, 200), m.message_id);
      await handleOptOut(lead);
    } else if (command === "/planos" || command === "/vip") {
