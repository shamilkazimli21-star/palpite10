...
export async function processPaymentSucceeded(data: unknown, options: { forceLead?: Lead } = {}): Promise<string> {
  ...
  if (!lead) {
    await notifyAdmin(
      `💰❓ Payment ${refs.paymentId} (${currency} ${amount.toFixed(2)}, ${plan?.name ?? refs.planId ?? "unknown plan"}, ${refs.email ?? "no email"}) could not be linked to a Telegram lead.\n` +
        `Find the customer and run:\n/link ${refs.paymentId} <telegram_id>\nto deliver VIP access and count the sale.`,
    );
    return "unlinked";
  }
  ...
  if (!alreadyCounted) {
    ...
    await recordMessage(lead.id, "event", `Pagamento confirmado pela Whop (${plan?.name ?? "VIP"}${renewal ? ", renovação" : ""}).`);
    ...
    await notifyAdmin(
      `💰 ${renewal ? "Renewal" : "NEW SALE"}: ${lead.first_name ?? "Lead"} (@${lead.username ?? "-"}) — ${plan?.name ?? "VIP"} ${currency} ${amount.toFixed(2)}` +
        `${lead.campaign ? ` · campaign ${lead.campaign}` : ""}${matchedBy === "recent_checkout" ? "\n⚠️ Linked by recent checkout (no metadata on the payment). Please double-check it is the right person." : ""}`,
    );
  }
...
export async function processRefund(data: unknown): Promise<string> {
  ...
  await notifyAdmin(`↩️ Refund: ${lead.first_name ?? lead.id} (@${lead.username ?? "-"}) — ${row.amount}. VIP access removed.`);
  return "refunded";
}
...
export async function processMembership(data: unknown, active: boolean): Promise<string> {
  ...
  await notifyAdmin(`👋 VIP ended: ${lead.first_name ?? lead.id} (@${lead.username ?? "-"}). Removed from the VIP channel.`);
  return "revoked";
}
...
export async function linkPaymentToLead(paymentId: string, lead: Lead): Promise<string> {
  ...
  if (!row) return `Payment ${paymentId} not found.`;
  if (row.lead_id) return `Payment ${paymentId} is already linked.`;
  const result = await processPaymentSucceeded(row.raw, { forceLead: lead });
  return `Payment ${paymentId} linked to ${lead.first_name ?? lead.telegram_user_id} (${result}). VIP access sent.`;
}
