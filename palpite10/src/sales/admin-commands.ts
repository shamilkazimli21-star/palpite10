...
const HELP = `PALPITE10 admin

Numbers
/stats [days] — funnel for the period (default 7)
/funnel — all-time funnel with step conversion
/campaigns [days] — results per campaign / ad
/objections [days] — what people push back on

Learning
/playbook [N] — read the active (or version N) playbook
/proposals (or /playbooks) — playbooks waiting for your decision
/approve N · /reject N
/experiments — A/B tests and their numbers
/activate N · /stopexp N
/learn — run the learning cycle now
/followups — run follow-ups now (ignores quiet hours)

People
/say <telegram_id> <text> — send a message as the bot
/done <telegram_id> — clear the "needs human" flag
/link <payment_id> <telegram_id> — attach an unlinked Whop payment and deliver VIP`;
...
export async function handleAdminCommand(chatId: number | string, text: string): Promise<boolean> {
  ...
  if (!reply) return false; // not an admin command → let /start, /planos etc. work for the owner too
