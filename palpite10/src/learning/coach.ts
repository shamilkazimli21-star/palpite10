...
export async function runCoach(options: { force?: boolean } = {}): Promise<CoachResult> {
  ...
  if (!parsed.success) {
    ...
    await notifyAdmin(`🧠 Learning batch #${batch.id}: coach output was malformed and was discarded. Playbook v${active.version} stays.`);
  ...
  if (problems.length) {
    await db().from("learning_batches").update({ result: "rejected", problems }).eq("id", batch.id);
    await notifyAdmin(`🧠 Learning batch #${batch.id}: proposal REJECTED by safety checks:\n- ${problems.join("\n- ")}\nPlaybook v${active.version} stays.`);
    return { status: "rejected", problems };
  }
