-- =====================================================================
--  PALPITE10 — update 3: analytics dashboard (ad spend, daily snapshots).
...
begin
  ...
  select (created_at at time zone 'America/Sao_Paulo')::date, coalesce(campaign, ''),
...
returns json language sql stable as $$
  select json_build_object(
    ...
    'hours', (select coalesce(json_agg(t), '[]'::json) from (select extract(hour from created_at at time zone 'America/Sao_Paulo')::int as k, count(*) as v from messages where role = 'user' and created_at > now() - interval '30 days' group by 1 order by 1) t),
